...
object RunMetis extends App {
  ...
  def prepareInput() = {
    ...
    Source.fromFile("/Users/harper/ResearchData/wiki-Vote.txt").getLines().filter(!_.startsWith("#")).foreach(s => {
      var parts = s.split("\\s+")
    ...
    var out = new PrintWriter(new FileOutputStream("wikivote_metis_input"));
    var dict = new PrintWriter(new FileOutputStream("wikivote_metis_dict"));
  ...
  def translateInput(): Unit = {
    ...
    var out = new PrintWriter(new FileOutputStream("wikivote_metis"));
    Source.fromFile("wikivote_metis_dict").getLines().foreach(s => {
      var parts = s.split("\t");
      dict += ((parts(1).toInt -> parts(0).toInt));
    });
    var counter = 1;
    Source.fromFile("wikivote_metis_output").getLines().foreach(s=>{
      var p = s.toInt;
