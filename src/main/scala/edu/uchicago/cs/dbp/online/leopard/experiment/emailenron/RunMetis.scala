...
object RunMetis extends App {
  ...
  def prepareInput() = {
    ...
    var out = new PrintWriter(new FileOutputStream("leopard/emailenron/metis_input"));
    var dict = new PrintWriter(new FileOutputStream("leopard/emailenron/metis_dict"));
  ...
  def translateInput(): Unit = {
    ...
    var out = new PrintWriter(new FileOutputStream("leopard/emailenron/p_metis"));
    Source.fromFile("leopard/emailenron/metis_dict").getLines().foreach(s => {
      var parts = s.split("\t");
      dict += ((parts(1).toInt -> parts(0).toInt));
    });
    var counter = 1;
    Source.fromFile("leopard/emailenron/metis_output").getLines().foreach(s => {
      var p = s.toInt;
